...
class _AdBannerState extends State<AdBanner> with WidgetsBindingObserver {
  ...
  Future<void> _initializeAdService() async {
    ...
    if (!AdFactory.isAdSupportedPlatform) {
      _logger.info('Ads not supported on platform: ${AdFactory.currentPlatformName}');
      return;
    }
    
    try {
      _logger.info('Initializing ad service for ${AdFactory.currentPlatformName}');
      ...
      _logger.severe('Exception during ad service initialization: $error', error, stackTrace);
  ...
  Widget build(BuildContext context) {
    ...
    if (_initializationFailed) {
      return widget.showDebugInfo ? _buildErrorState() : const SizedBox.shrink();
    }
