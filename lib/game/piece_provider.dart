...
class ForcedSequenceProvider implements IPieceProvider {
  ...
  static ForcedSequenceProvider fromState(
    ...
    IPieceProvider baseProvider
  ) {
    final forcedType = TetrominoType.values
        .firstWhere((t) => t.name == state['forcedType']);
...
class PieceProviderStack {
  ...
  void _cleanupExhausted() {
    ...
    _stack.removeWhere((provider) {
      ...
      if (exhausted) {
        debugPrint('PieceProviderStack: Removed exhausted ${provider.description}');
      }
      return exhausted;
    });
    
    if (_stack.length != initialCount) {
      debugPrint('PieceProviderStack: Cleaned ${initialCount - _stack.length} exhausted providers');
    }
  }
  ...
  String get stackDescription {
    if (_stack.isEmpty) return 'Empty stack (using ${_baseProvider.description})';
  ...
  void restoreState(Map<String, dynamic> state) {
    ...
    for (final stackState in stackStates) {
      ...
      if (type == 'ForcedSequence') {
        final provider = ForcedSequenceProvider.fromState(providerState, _baseProvider);
        _stack.add(provider);
      }
    }
    
    debugPrint('PieceProviderStack: Restored state with ${_stack.length} providers');
  }
}